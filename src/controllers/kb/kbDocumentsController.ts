...
export const deleteKnowledgeBaseDocument = async (req: Request, res: Response) => {
  ...
  try {
    const doc = await documentRepository.findOne({ where: { id: documentId, knowledgeBaseId: kbId } });
    if (!doc) {
...
export const reindexKnowledgeBase = async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (documents.length === 0) return res.status(400).json({ message: 'No documents found for this knowledge base' });
    ...
    for (const doc of documents) {
      try {
        ...
        if (!pdfPath) {
          errors.push({ documentId: doc.id, fileName: doc.fileName, error: 'File not found on disk' });
          continue;
        }
  ...
  try {
    ...
    if (user.role !== 'ADMIN') {
      const license = await licenseRepository
        ...
        .getOne();
      if (!license || !isLicenseValid(license)) return res.status(403).json({ message: 'License is not valid' });
      const hasKb = Array.isArray(license.knowledgeBases) && license.knowledgeBases.some((kb) => kb.id === kbId);
      if (!hasKb) return res.status(403).json({ message: 'Knowledge base not attached to this license' });
    }
