...
export class RagService {
  ...
  async pickBestKnowledgeBase(kbIds: string[], question: string): Promise<string | null> {
    ...
    if (!best.kbId || !Number.isFinite(best.score) || best.score === Number.NEGATIVE_INFINITY) return null;
    return best.kbId;
  }
  ...
  async queryAcrossKnowledgeBases(
    ...
    const uniqueKbIds = Array.from(new Set(kbIds)).filter(Boolean);
    if (uniqueKbIds.length === 0) return await this.queryGlobal(question, promptInstructions, historyRaw);
    if (uniqueKbIds.length === 1) return await this.query(uniqueKbIds[0], question, promptInstructions, historyRaw);
    ...
    const merged = perKbResults.flat().sort((a, b) => b[1] - a[1]).slice(0, topK);
    if (merged.length === 0) {
      return await this.queryGlobal(question, promptInstructions, historyRaw);
    }
